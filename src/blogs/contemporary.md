...
I did a little bit of investigation of my own and found a [BCI demonstration in Godot](https://medium.com/@nerveinvader/controlling-godot-with-brainwaves-in-less-than-100-lines-of-code-4465982a21eb), my engine of choice for short-term projects. Through the demo I also found online repositories of recordings of EEG data, including data from participants clenching their fists, which was the action that I wanted our game to listen for. If I had been working solo on these projects then I'd have switched from Unity to Godot immediately and used the existing training data to pre-train our EEG model. However, I'd already hogged most of the programming work for myself in the other projects and Matt and Conall were proficient in C#, so I didn't try switching the team over. I also suspect that I would've been on my own in terms of writing scripts or finding existing software to pipe the BCI data into Godot.

Only in the third week did we get access to BCI caps, and they were different to the ones we were shown in previous classes. These FlexEEG caps from NeuroCONCISE (a company founded by UU Magee staff) had only seven electrodes, with six designed to read the primary motor cortex, and the last reading baseline activity from the premotor cortex. This meant more complicated tasks like estimating emotional states were not possible, our idea relied on motor cortex readings anyway, so I believed our group would be fine.

### Technical Difficulties
Actually using the BCI caps proved difficult. I was the first to go through the process: Sarshar worked with me to set up a Rube Goldberg machine of software to make the FlexEEG work and pass its readings to Unity. I had to run several different pieces of software at once, including switching between two different versions of the same crusty old software *and* a seemingly vibe-coded clone, all while juggling an extremely spotty Bluetooth connection that would drop out and require the whole chain of software to be reset before the headset would connect again. This experience only strengthened my preference for wired connections to troublesome hardware (including printers, which are all terrible and should be wired or incinerated). It took almost the whole day to set up. At the end of the day, Conall kindly volunteered to be gelled up to wear the headset for 20 minutes of model training, and... it did not work. We had no idea why, but we had no usable model. I quite frankly considered the whole day a waste of time, and we didn't have the rest of the week to fix things as we couldn't take the headsets home. 